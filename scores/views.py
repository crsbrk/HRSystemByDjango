...
def index(request):
    ...
    scores = Scores.objects.all().filter(score_year_month__contains='2018')# the final scores of a month
...
def updateScoreOfWorkers(myYear, myMonth):
    ...
    try:
        ...
        cutovers = Cutovers.objects.filter(deadline_at__year = myYear, deadline_at__month = myMonth)
        orders = Orders.objects.filter(deadline_at__year = myYear, deadline_at__month = myMonth)
        bonuses = Bonuses.objects.filter(created_at__year = myYear, created_at__month = myMonth)
